...
const Dashboard: React.FC = () => {
  ...
  console.log({ currentUser, roles, organizationId, propertyId });
    ...
    <Box sx={{ p: 3 }}>
      ...
      {roles.includes('property_manager') && (
        <Typography variant='h6' color='secondary' sx={{ mt: 2 }}>
          You are a Property Manager for Organization ID:{' '}
          {organizationId || 'N/A'}.
        </Typography>
      )}
      ...
      {roles.includes('resident') && (
        <Typography variant='h6' color='info' sx={{ mt: 2 }}>
          You are a Resident. Your Organization ID: {organizationId || 'N/A'},
      ...
      <Typography variant='body2' sx={{ mt: 2 }}>
        This is a protected route. Your access is determined by your roles and
        assigned IDs.
      </Typography>
    </Box>
  );
