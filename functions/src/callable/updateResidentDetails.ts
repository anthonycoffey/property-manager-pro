...
export const updateResidentDetails = onCall(async (request) => {
  ...
  const { organizationId, propertyId, residentId, updatedData } = request.data as UpdateResidentData;
  ...
  if (callerRoles.includes('property_manager') && authorized) { // Re-check if authorized was set to true by PM role
    const propertyRef = db.doc(`organizations/${organizationId}/properties/${propertyId}`);
    const propertyDoc = await propertyRef.get();
    if (!propertyDoc.exists || propertyDoc.data()?.managedBy !== callerUid) {
  ...
  const residentDocRef = db.doc(`organizations/${organizationId}/properties/${propertyId}/residents/${residentId}`);
  ...
  try {
    ...
    const allowedFields: Array<keyof typeof updatedData> = [
      ...
      'vehicleColor',
      'licensePlate',
      // Add other resident-profile editable fields here
    ];
    ...
    for (const key of allowedFields) {
      if (Object.prototype.hasOwnProperty.call(updatedData, key)) {
        ...
        if (key === 'leaseStartDate' || key === 'leaseEndDate') {
          if (value === null || value === undefined) { // Handle undefined as well for explicit clear
        ...
        if (value !== undefined || (fieldProcessed && (key === 'leaseStartDate' || key === 'leaseEndDate'))) {
          hasValidUpdate = true;
        }
      }
    }
    ...
    if (!hasValidUpdate) {
      throw new HttpsError('invalid-argument', 'No valid fields or actual changes provided for update.');
    }
    ...
    console.error('Error updating resident details:', e.message, e.code ? `(${e.code})` : '', e);
    throw new HttpsError('internal', e.message || 'An internal error occurred while updating resident details.');
  }
});
