...
const ViolationsDashboardPage = () => {
  ...
  if (!selectedPropertyId) {
    return (
      <Box>
        <Typography>Please select a property from the dashboard to view violations.</Typography>
      </Box>
    );
  }
          ...
          <FormControl sx={{ minWidth: 120 }}>
            ...
            <FormControl sx={{ minWidth: 180 }}>
              ...
              >
                ...
                <MenuItem value="unauthorized_space">Unauthorized Space</MenuItem>
                ...
                <TableBody>
                  {violations.length === 0 ? (
                    ...
                    violations.map((violation) => {
                      ...
                      let date: Date;
                      if (violation.createdAt && 'toDate' in violation.createdAt) {
                        // It's a Firestore Timestamp instance
                        date = violation.createdAt.toDate();
                      } else if (violation.createdAt && '_seconds' in violation.createdAt) {
                      ...
                      return (
                        <TableRow key={violation.id}>
                          <TableCell>{!isNaN(date.getTime()) ? date.toLocaleString() : 'Invalid Date'}</TableCell>
                          <TableCell>
                            {profilesLoading ? (
                              ...
                              profiles[violation.reporterId] ? (
                                <Chip
                                  avatar={<Avatar alt={profiles[violation.reporterId].displayName} src={profiles[violation.reporterId].photoURL || undefined} />}
                          ...
                          <TableCell align='center'>{violation.licensePlate}</TableCell>
                          <TableCell align='center'>
                            <Chip label={violation.violationType.replace(/_/g, ' ').toUpperCase()} />
                          </TableCell>
                          <TableCell align='center'>
                            <Chip label={violation.status.toUpperCase()} color={
                              violation.status === 'resolved' ? 'success' : violation.status === 'acknowledged' ? 'warning' : 'default'
                            } />
                          </TableCell>
                        </TableRow>
                      );
