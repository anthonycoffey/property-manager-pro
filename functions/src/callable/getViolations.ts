...
export const getViolations = onCall(async (request) => {
  ...
  const { propertyId, status, type, startDate, endDate, page, rowsPerPage } = request.data;

  if (!propertyId) {
    throw new HttpsError('invalid-argument', 'The function must be called with a "propertyId".');
  }

  try {
    let query: Query = db.collection('violations').where('propertyId', '==', propertyId);
