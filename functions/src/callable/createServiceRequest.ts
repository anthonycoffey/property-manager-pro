...
import type { ServiceRequest, ServiceRequestStatus, Resident } from '../types.js'; // Import from local types.ts
...
export const createServiceRequest = https.onCall(
  async (data: CreateServiceRequestData, context: CallableContext) => {
    ...
    const { organizationId: tokenOrgId, propertyId: tokenPropId, roles, name: residentNameFromToken } = context.auth.token;
    ...
    if (!data.requestType || !data.serviceLocation?.trim() || !data.serviceDateTime) {
        throw handleHttpsError(
            'invalid-argument',
    ...
    const residentDocRef = db.doc(`organizations/${data.organizationId}/properties/${data.propertyId}/residents/${residentId}`);
    try {
        ...
        if (residentDocSnap.exists()) {
            ...
            console.warn(`Resident document not found for ${residentId} during service request creation. Using token name.`);
        }
    } catch (err) {
    ...
    const serviceRequestCollectionRef = db.collection(`organizations/${data.organizationId}/services`);
    ...
    const newServiceRequest: Omit<ServiceRequest, 'id'> = {
        ...
        serviceDateTime: data.serviceDateTime ? new Date(data.serviceDateTime) : FirebaseAdminFieldValue.serverTimestamp(), // Store as Timestamp or keep as ISO string
    ...
    try {
      ...
      return { success: true, serviceRequestId: docRef.id, message: 'Service request created successfully.' };
    } catch (error) {
      console.error('Error creating service request:', error);
      throw handleHttpsError('internal', 'An internal error occurred while creating the service request.');
    }
  }
);
